...
class BarCode @Inject()(cardService: service.common.CardService) extends BaseController {
  def generateBarCode(content: String): Array[Byte] = {
    ...
    val dimensions = bc.calcDimensions(content)
    val image = new BufferedImage(dimensions.getWidth.toInt + 1, dimensions.getHeight.toInt + 1, BufferedImage.TYPE_INT_RGB)
