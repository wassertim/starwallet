 ...
 * }}}
 */
object DAssets extends CustomAssetsBuilder
//This assets builder was modified in order to map static files directly which is better for development when you change them often
class CustomAssetsBuilder extends Controller {
  ...
  private def playResource(resourceName: String) = {
    if (!Play.isProd)
      Some(new java.net.URL("file:" + Play.getFile(resourceName).getAbsolutePath))
    else Play.resource(resourceName)
  }
      ...
      resourceNameAt(path, file).map { resourceName =>
        ...
        val resource = {
          gzippedResource.map(_ -> true)
            ...
            .orElse(playResource(resourceName).map(_ -> false))
        }
